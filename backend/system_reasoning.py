...
def build_reasoning_trace(deterministic: dict, triage: dict, differential: dict, protocol: dict) -> list[dict]:
    trace: list[dict] = []
    ...
    return trace
def build_reasoning_trace(deterministic, triage, differential, protocol):
    trace = []
    ...
    if protocol.get("protocol_matches"):
        trace.append({
            "type": "protocol",
            "message": f"Protocol-backed actions identified"
        })
