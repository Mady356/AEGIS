...
# ---------------------------------------------------------------------
def _reasoning_kwargs(reasoning_effort: str | None) -> dict:
    """Build the reasoning_effort kwarg for the OpenAI SDK call.

    Ollama's OpenAI-compat layer honors `reasoning_effort` (verified:
    `reasoning_effort: "none"` cuts Gemma 4's CoT entirely, sub-second
    response). Pass `None` or "default" / "" to leave the model's
    default behavior in place (used for the structured /api/crisis
    bundle where reasoning helps).
    """
    eff = reasoning_effort
    if eff is None:
        eff = config.LLM_REASONING_EFFORT_DEFAULT
    if not eff or str(eff).lower() in ("default", "none-default"):
        return {}
    return {"extra_body": {"reasoning_effort": str(eff).lower()}}
...
async def call_llm_json(
    ...
    `reasoning_effort` defaults to ``config.LLM_REASONING_EFFORT_DEFAULT``
    (default "none" — fastest). Pass ``"default"`` or ``""`` to keep
    the model's chain-of-thought enabled (used by /api/crisis where
    structured reasoning pays for itself).
...
async def chat_completion(
    ...
    """Multi-turn chat: pass a full messages list (system + history +
    new user turn). Returns (assistant_content, metadata).

    Defaults to ``reasoning_effort="none"`` via ``call_llm_json``'s
    config-driven default — sub-second responses on warm Gemma 4
    26B-A4B. Raises LLMError on transport / empty-response failures."""
    global LAST_LATENCY_MS, LAST_PROMPT_TOKENS, LAST_COMPLETION_TOKENS

    temperature = (temperature if temperature is not None
                   else config.LLM_TEMPERATURE_NARRATIVE)
    if max_tokens is None:
        max_tokens = 512  # tight budget — chat turns should be brief
    ...
    try:
        ...
        try:
            ...
            LOG.error("chat_completion timed out after %.1fs",
                      config.LLM_REQUEST_TIMEOUT_SECONDS)
            raise LLMError(f"LLM timed out: {exc}") from exc
        except APIError as exc:
            LOG.error("chat_completion API error: %s", exc)
            raise LLMError(f"LLM API error: {exc}") from exc
    except ImportError as exc:
        raise LLMError(str(exc)) from exc
    ...
    if not content.strip():
        # Empty content despite reasoning_effort="none" → model went
        # off-rails or context exhausted; surface for the handler to
        # fall back gracefully rather than silently sending empty UI.
    ...
    LAST_PROMPT_TOKENS = metadata["prompt_tokens"]
    LAST_COMPLETION_TOKENS = metadata["completion_tokens"]
    LOG.info("chat_completion: %dms · %s prompt / %s completion tokens",
             latency_ms, metadata["prompt_tokens"], metadata["completion_tokens"])
